    ...
    public Dimension render(Graphics2D graphics) {
        ...
        if (config.maxHit()) {
            panelComponent.getChildren().add(LineComponent.builder()
                    .left("Max Hit:")
                    .right(Double.toString(plugin.getEquippedWeaponMaxHit()))
                    .build());
        }

        if (config.showSpec() && plugin.maxHitSpec(plugin.weaponName(), plugin.getEquippedWeaponMaxHit()) != -1) {
            panelComponent.getChildren().add(LineComponent.builder()
                    .left("Max Special:")
                    .right(Double.toString(Math.floor(plugin.maxHitSpec(plugin.weaponName(), plugin.getEquippedWeaponMaxHit()))))
                    .build());
        }
        ...
        HashMap<String, InventoryWeapon> map = plugin.equipableItems();
        if (map == null) {
